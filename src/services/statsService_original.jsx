  ...
  async getActiveSessions() {
    try {
      if (this.isFirebaseConnected) {
        ...
        const now = Date.now();
        const thirtySecondsAgo = now - (30 * 1000); // 🔥 30초로 확대 (더 관대하게)
        ...
        const activeSessions = sessions.filter((session) => {
          ...
          const isRecent = heartbeatTime > thirtySecondsAgo;
          return isRecent;
        });
  ...
  onActiveSessionsChange(callback) {
    if (this.isFirebaseConnected) {
      ...
      const listener = onValue(sessionsRef, async (snapshot) => {
        // 디바운싱: 1초 내에 여러 변경이 있으면 마지막것만 처리
        if (debounceTimer) {
